\section{Dynamic simulation of HSA robots}\label{sec:hsamodel:simulation}
...
%
We give some background on the \gls{DCM} in Section~\ref{sub:hsamodel:simulation:discretized_cosserat_rod_model}. Then, in \ref{sub:hsamodel:simulation:auxetic_trajectory}, we propose a mechanism to infuse the auxetic trajectory for a \gls{HSA} into the \gls{DCM} framework. Subsequently, we verify the steady-state behaviour of an \gls{HSA} against the mechanical characteristics in \ref{sub:hsamodel:simulation:verification_good}. Next, we lay out in \ref{sub:hsamodel:simulation:hsa_robots} the necessary boundary conditions of the \glspl{HSA} and describe the joint mechanism connecting the platform with the rods. Finally, we explain in Section~\ref{sub:hsamodel:simulation:motion_primitives} how we were able to reproduce in simulation the main motion primitives of \gls{HSA} robots.

\subsection{Background: Discretized Cosserat-rod model}\label{sub:hsamodel:simulation:discretized_cosserat_rod_model}
...
\subsection{Auxetic trajectory}\label{sub:hsamodel:simulation:auxetic_trajectory}
...
\subsection{Verification of single HSA steady-state behaviour}\label{sub:hsamodel:simulation:verification_good}
...
\subsection{Simulating HSA robots: boundary conditions and joints}\label{sub:hsamodel:simulation:hsa_robots}
...
\subsection{Qualitative evaluation of motion primitives in simulation}\label{sub:hsamodel:simulation:motion_primitives}

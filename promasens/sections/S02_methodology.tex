...
% For the sake of clarity, we use the \gls{PCC} kinematic state parametrization as proposed by \citep{della2020improved}. We consider a continuum robot described by $n_\mathrm{b}$ \gls{CC} segments and configuration variables $q \in \mathbb{R}^{3n_\mathrm{b}}$. Note, however, that the proposed method applies to any finite-dimensional kinematic description of a soft robot~\citep{armanini2023soft}.
...
% \begin{equation}
%     q_i = \begin{bmatrix}\Delta_{x,i} & \Delta_{y,i} & \delta L_i \end{bmatrix}^{\top} \in \mathbb{R}^3
% \end{equation}
...
% We extend this parametrization as introduced by \citet{della2019control, della2020soft} to 3D environments by specifying the azimuth angle of the bending direction $\phi(t)$ and additionally allowing for an elongation $\delta L(t)$ of the segment. 
...
% \begin{equation}
%     \xi_j^{k} = \begin{pmatrix} d_j^k & \alpha_j^k & \beta_j^k & \theta_j^k \end{pmatrix}^\top.
% \end{equation}
...
% We compute the translational and rotational components of the transformation matrix $p_{i-1}^{\mathrm{m}_k}$ and $R_{i-1}^{\mathrm{m}_k}$ \textcolor{blue}{by evaluating the forward kinematics $T_{i-1}^v(q_i,v)$, which are reported in Appendix~\ref{sub:promasens:kinematics}, at the point $v = \frac{d_{\mathrm{m}_k}}{L_{0,i}}$.}

% as in \citep{della2020improved} by substituting $\Delta_{x}^{\mathrm{m}_k} = \frac{d_{\mathrm{m}_k}}{L_{0,i}} \, \Delta_{x,i}$, $\Delta_{y}^{\mathrm{m}_k} = \frac{d_{\mathrm{m}_k}}{L_{0,i}} \, \Delta_{y,i}$ and $\Delta_{\mathrm{m}_k} = \frac{d_{\mathrm{m}_k}}{L_{0,i}} \, \Delta_{i}$. % in \eqref{eq:promasens:transform_improved_q}.
...
% by scaling the configuration vector $q_i$ with $\frac{d_{\mathrm{s}_j,\mathrm{a}}}{L_{0,i}}$ in the transformation matrix~\citep{della2020improved}.
...
\begin{equation}
    ...
    \quad
    \cos (\alpha_{j}^k) = \{ o_{\mathrm{m}_k} \}_{0} \cdot \{ o_{\mathrm{s}_j} \}_{0}.
\end{equation}
...
\begin{equation}\label{eq:promasens:theta_jk_and_beta_jk}
    \cos (\beta_j^k) = \frac{p_j^{k} \cdot \{ o_{\mathrm{s}_j} \}_{0}}{\lVert p_j^{k} \rVert_2},
    \qquad
    \cos (\theta_j^k) = \frac{p_j^{k} \cdot \{ o_{\mathrm{m}_k} \}_{0}}{\lVert p_j^{k} \rVert_2}.
\end{equation}
...
\begin{algorithmic}[1]
    ...
    \Comment{Estimated configuration at the last time step}
    \Require $f_\xi: \mathbb{R}^{2n_\mathrm{b}} \rightarrow \mathbb{R}^{n_\mathrm{s} + 4n_\mathrm{m} n_\mathrm{s}}$ \Comment{Magnet sensor kinematics}
    \Require $f_\pi: \mathbb{R}^{n_\mathrm{s} + 4n_\mathrm{m} n_\mathrm{s}} \rightarrow \mathbb{R}^{n_\mathrm{s}}$ \Comment{Trained neural network}
    \Require $\gamma \in \mathbb{R}$ \Comment{Step size}
    ...
    \While{$l < n_\mathrm{it}$}
        ...
        \vspace{0.25em}
        % \STATE $\frac{\partial}{\partial \hat{q}_l} \mathcal{L}_{u}(\hat{q}_l) \gets \frac{2}{n_\mathrm{s}} \frac{\partial }{\partial {\hat{q}_l}} f_{\mathrm{\xi}}^\top(\hat{q}_l) \, \frac{\partial}{\partial {\hat{\xi}_l}} f_\pi^\top(\hat{\xi}_l) \, (\hat{u}_l - u(t))$
...
During training on a dataset of length $n_\mathrm{t}$, we minimize the \gls{MSE} between the predicted sensor measurement $\hat{u}_j$ and the actual sensor measurement $u_j$:
\begin{equation}

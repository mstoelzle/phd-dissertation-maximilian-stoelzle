...
We now leverage an encoder-decoder architecture to map these high-dimensional observations into a compressed latent space: % parametrized by $z^\top & \dot{z}^\top \end{bmatrix}^\top \in \mathbb{R}^{2n_z}$.
...
\subsubsection{Estimation of Initial Latent Velocity}\label{sub:con:initial_latent_velocity_estimation}
...
%     \caption{A planar soft robot, consisting of six segments, that deforms with piecewise constant curvature. The behavior of the simulated robots included in datasets \emph{PCC-NS-2} and \emph{PCC-NS-3} resembles this pneumatically actuated robot. The source of the image is \citet{della2020model}.}
...
\subsubsection{Unactuated PDE Reaction-Diffusion Dataset}\label{ssub:con:reaction_diffusion_dataset}
 We consider the \nth{1}-order Reaction-diffusion (\emph{R-D}) \gls{PDE} on which \citet{champion2019data} evaluated their SINDy Autoencoder on. The \gls{PDE} of the high-dimensional lambda-omega reaction-diffusion system is defined as
\begin{equation}
\begin{split}
